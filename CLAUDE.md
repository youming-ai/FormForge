...
- 默认模型 `qwen/qwen3-30b-a3b-2507`（Qwen3-30B-A3B MoE，30B 总参仅 **3B 激活**、tool calling 已验证）——多轮 agent 循环选 MoE 而非稠密 27B（如 Qwen3.8-27B，慢数倍）；填 `auto` 则调 `/v1/models` 自动选「已加载」的对话模型（排除 embed/asr/rerank）。
