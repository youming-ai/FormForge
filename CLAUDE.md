...
- 默认模型 `unsloth/gemma-4-26B-A4B-it-GGUF:gemma-4-26B-A4B-it-UD-Q4_K_M`（Gemma 4 26B-A4B MoE，**4B 激活**）——多轮 agent 循环选 MoE 而非稠密 27B（如 Qwen3.8-27B，慢数倍）；面板直接默认填入，支持手动改写。
...
- **确认页判定**：范围内没有可编辑控件 + 页面出现「最终提交」类按钮（`SUBMIT_WORDS` 词表）→ 视为确认页。`click_button` 在确认页**硬拦截**，只允许 `finish`，绝不提交。
...
- **ref 失效**：SPA 切步骤后 DOM 被重建，`getRef` 会校验 `item.isConnected`，失效返回「请重新 get_form」明确提示，避免静默写孤儿节点。
