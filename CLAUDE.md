...
- 默认模型 `unsloth/gemma-4-26B-A4B-it-GGUF:gemma-4-26B-A4B-it-UD-Q4_K_M`（llama.cpp GGUF 版，未加载时 llama-swap 按需 JIT 换入，首次有冷加载延迟；同服务器已加载可用的替代：`unsloth/Qwen3.8-27B-GGUF:8-27B-Q4_K_M` 已验证 tool calling）；填 `auto` 则调 `/v1/models` 自动选已加载模型。
