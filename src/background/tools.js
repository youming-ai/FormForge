...
export const TOOLS = [
  ...
  fn('read_options',
    ...
    ['ref']),
  fn('fill_text',
    '向文本框/多行文本框/富文本(contenteditable)填值。按字段语义与表单语言填合理测试值（日文表单填地道日语，英文填英文，中文填中文）。',
    { ref: { type: 'string' }, value: { type: 'string' } },
    ['ref', 'value']),
  fn('choose_option',
    '为 select/radio/checkbox 选一项。select/radio 传 option=标签文本或 value；select 传 option="first" 直接选第一个可用项（必填校验只要求非空时的最快路径，不确定选什么就用它）；checkbox 传 option="check" 或 "uncheck"(同意条款一律 check)；多选下拉(multiple=true)可多次调用选多个。需要特定选项时先 read_options。',
