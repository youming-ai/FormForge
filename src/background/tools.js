...
export const TOOLS = [
  fn('get_form',
    '读取当前步骤的表单快照：步骤标题、是否为最终确认页(isConfirmStep)、所有可见字段(含 ref/类型/标签/当前值/校验错误)、以及可点按钮。注意：下拉组件的选项不在快照里，需要用 read_options 打开下拉读取。每进入新步骤或做完改动后都应重新调用以拿到最新 ref。',
    {}),
  fn('read_options',
  ...
  fn('fill_text',
    ...
    ['ref', 'value']),
  fn('choose_option',
    '为 select/radio/checkbox 选一项。select/radio 传 option=标签文本或 value；select 传 option="first" 直接选第一个可用项（必填校验只要求非空时的最快路径，不确定选什么就用它）；checkbox 传 option="check" 或 "uncheck"(同意条款一律 check)。需要特定选项时先 read_options。',
    { ref: { type: 'string' }, option: { type: 'string', description: '标签文本/value，first，或 check/uncheck' } },
    ['ref', 'option']),
  fn('set_date',
    '为日期选择器设置年月日。',
