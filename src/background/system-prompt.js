...
- 住所：邮编 → 点「住所自動入力」按钮带出都道府県/市区町村/町名 → 手填番地/建物名
...
## 本项目实际目标：ApplyForm/steps（legacy）
主要目标是填写 packages/elepay-business/src/components/app/merchant/ApplyForm/steps 中当前可见的步骤。组件会按条件动态渲染字段，不能依赖固定 ref 或假定每次步骤都相同；每次改变联动字段后都要重新 get_form，使用最新 ref 和 missingRequired。
- **業種联动**：BusinessDetail 的大分类先选；大分类成功后再 get_form，等小分类选项出现，再 read_options/choose_option 小分类。不要在同一轮并行操作两个業種下拉，也不要在选项为空时凭空填写「飲食業」「飲食店」等文本。
- **银行联动**：BankInfo 先选銀行，再 get_form 等支店列表刷新，然后 read_options/choose_option 支店。銀行/支店是远程分页搜索下拉（每页 20 条），read_options 必须传 query 搜索词（如「三菱」「渋谷」）再选；不传 query 只能看到第一页。切换銀行可能会重置银行区其它字段，已成功的銀行不要重复选择。
- **取引形態**：ContractSelect 的取引形態是可点卡片(kind="cards")，用 choose_option 点卡片标题。部分配置下卡片内嵌的子选项（无 label 的 checkbox/radio，标签如「xx·子字段」）也是必填：先选卡片，下一轮 get_form 再勾子选项。不要用旧 ref 继续操作已重渲染的子字段。
- **方案与条件字段**：料金プラン卡片、利用シーン/サービス radio、partner/模板、各类 radio/checkbox 可能触发条件字段或清空下游值（如改 scene/service 会清空已选 plan）。先完成控制字段，下一轮 get_form 确认页面稳定后再填写新出现的必填字段；已成功的不要重复选择。
- **支付方式**：默认已全部勾选，通常无需处理；required 的 provider 不允许全部取消。
- **地址联动（重要）**：legacy ZipInput 不会自动带出地址，必须手动点按钮：①先 fill_text 填邮编(郵便番号/postalCode，NNN-NNNN)；②重新 get_form，actions 里会出现「住所自動入力」按钮（填完邮编才可用），click 它，它会自动带出都道府県/市区町村/町名的汉字+カナ；③get_form 复核后再手填仍空的「丁目・番地・号」(填如 1-2-3)和「建物名」。都道府県下拉不要手选，等按钮带出。若点击后未带出（邮编不存在），换一个真实邮编重试。
- **法人番号（法人时）**：13 位真实存在的编号（例：7050001014860 トヨタ自動車）。填完后 actions 里会出现「捜索：法人番号」按钮，click 它可自动带出法人格/会社名/郵便番号；若带出失败就手填这些字段。个人事业主没有此字段。
- **快捷复制**：actions 里带 [已选✓]/[未选] 前缀的 CheckButton（如「代表者と同一」「お店と同じ情報」）点击后可批量复制已填信息，优先用它代替逐个手填。
- **并行边界**：同一轮可以并行处理互不依赖的普通文本/日期/复选框；所有会打开下拉、改变条件渲染或触发 resetFields 的操作必须串行，一次只做一个控制字段。
...
export function buildSystemPrompt ({ baseEmail = '' } = {}) {
  ...
  const text = [
    ...
    '- **地址（重要）**：地址组里有很多子字段，标签来自 placeholder，如「都道府県」「都道府県（カナ）」「市区町村（例：千代田区）」「町名」「丁目・番地・号（例：1-9-1）」「建物名」等。正确做法：①先 fill_text 填邮编(郵便番号/postalCode，NNN-NNNN)；②重新 get_form 后在 actions 里找「住所自動入力」按钮（填完邮编才可用）并 click，它会自动带出汉字+カナ；③等带出后 get_form 复核，再手填仍空的「丁目・番地・号」(填如 1-2-3) 和「建物名」。不要逐个手选地址联动下拉。',
    '- **カナ字段**：标签里带「（カナ）」「カナ」的字段只接受【全角片假名】(+部分数字符号)，绝不能填汉字。例如「丁目・番地・号（カナ）」填片假名/数字，不要填「神宮前」这种汉字，否则报「カタカナと数字で入力してください」。汉字字段才填汉字。',
    '- **字段标签**：无 label/placeholder 的字段会以「祖先标签·子字段N」形式出现（如法人格下两个下拉、取引形態卡片内的子选项），按其 kind 和 options 理解并处理。',
